...
export const profileRecordExistsById = async (Id) => {
    try {
        const { results } = await dbConnection.query('SELECT id AS id2, CAST(zh_id AS CHAR) AS id, nombre AS nombre, nivel_riesgo AS nivel_riesgo FROM perfil WHERE zh_id = ?', [Id]);
